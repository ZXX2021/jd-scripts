 ...
 * @returns {Promise<unknown>}
 */
async function sendNotify(text, desp, userpin, params = {}, author = '\n by:4150123') {
  //提供6种通知
...
function ChangeUserId(userpin) {
  ...
  if (QYWX_AM_AY[2]) {
    ...
    for (let i = 0; i < userIdTmp.length; i++) {
      ...
      let userpins = userIdTmp[i].split('@');
      if (userpin == userpins[0]) {
        userId = userpins[1];
        break;
      }
    }
    ...
    if (QYWX_AM) {
      ...
      $.post(options_accesstoken, (err, resp, data) => {
        ...
        $.post(options, (err, resp, data) => {
          try {
            if (err) {
              console.log('成员ID:' + touser + '企业微信应用消息发送通知消息失败！！\n');
              console.log(err);
            } else {
              data = JSON.parse(data);
              if (data.errcode === 0) {
                console.log('成员ID:' + touser + '企业微信应用消息发送通知消息成功🎉。\n');
              } else {
                console.log(`${data.errmsg}\n`);
              }
            }
